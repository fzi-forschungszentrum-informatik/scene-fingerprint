...
import os
import numpy as np
from utils.csv_object_list_dataset_loader.src.csv_object_list_dataset_loader import Loader, Scenario  # noqa
...
class TrafficQuality(BaseMetric):
    ...
    def calculate_metric(self):
        ...
        for ego_object in road_objects_ssq:
            tq, _, _, _, _ = self.get_traffic_quality(ego_object.entity_id, self._timestamp)
            tqs[0, row] = tq
            row += 1
            ...
            if len(road_objects_dtq) <=1:
                tq, macro, micro_complete, \
                    nano_complete, individual_complete = self.get_traffic_quality(
                        ego_object.entity_id, self._timestamp)
                # tqs[0, row] = tq
                tqs[0, row] = macro
    ...
    def get_individual_traffic_quality(self, start_time_ms, end_time_ms, time_step_size, ego_id):
        ...
        mean_acc = 0.0
        old_ts = 0
        for ts in np.arange(start_time_ms, end_time_ms + time_step_size, time_step_size):
            if ts in self._scenario.timestamps:
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------
    dist = TrafficQuality(complete_scenario, timestamp)
    dist.calculate_metric()
    distance_matrix = dist.results_matrix
    dist.visualize_matrix(reverse=True)

    print(dist.accumulate_to_list(np.argmax))
    print(dist.accumulate_to_scalar(np.argmax))
