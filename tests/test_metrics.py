...
from scenario_criticality.traffic_quality.traffic_quality import TrafficQuality
...
class TestTrafficQuality(unittest.TestCase):
    ...
    def test_multiple_timestamps(self):
        validation_dict = {900: 0.00595485,
                           1900: 0.00190128,
                           2900: 1.53295037,
                           3900: 1.96238058,
                           4900: 1.55067991}
        result_dict = {}
        timestamps = [900, 1900, 2900, 3900, 4900]
        for ts in timestamps:
            tq = TrafficQuality(self.scenario, ts)
            tq.calculate_metric()
            result_dict[ts] = np.round(np.max(tq.results_matrix), 8)
        print(result_dict)
        self.assertDictEqual(result_dict, validation_dict)
